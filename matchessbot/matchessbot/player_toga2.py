...
from pychess.Players import UCIEngine

# TODO NOTE: DOES NOT WORK YET - NEEDS DEBUGGING!!!
# TODO: Rename this node/file to toga2!

class PlayerToga2(Node):
    def __init__(self):
        super().__init__('player_pychess')
        ...
        # self.get_logger().info('chess.engine.options: "%s"' % f"{chess.engine.INFO_BASIC}")
...
def main(args=None):
    ...
    player_pychess = PlayerToga2()

    rclpy.spin(player_pychess)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    player_pychess.destroy_node()
    rclpy.shutdown()
