...
def import_config(model_config_name: str="CT-E-20", run_number: int=0):
    if model_config_name=="CT-E-20":
        return {
    ...
    elif model_config_name == "CT-E-20-v2":
        return {
            "NAME": model_config_name,
            "VOCAB_SIZES": get_vocab_sizes(),
            "D_MODEL": 512,
            ...
            "BATCH_SIZE": 512,
            "NUM_WORKERS": 2,       # number of workers to use for dataloading
            "CHECKPOINT_FOLDER": "training_runs/" + model_config_name + "/model_checkpoints/",
            "DATA_FOLDER": "dataset/",
            "H5_FILE": "ML23c.h5", #"LE22ct-v2.h5",
            "LOGS_FOLDER": "training_runs/" + model_config_name + "/logs/",
            "MAX_MOVE_SEQUENCE_LENGTH": 10,
            "PREFETCH_FACTOR": 2,   # number of batches to prefetch per worker
            "PIN_MEMORY": False,    # pin to GPU memory when dataloading?"
            "SAMPLING_K": 1,        # k in top-k sampling model predictions during play
            "PRINT_FREQUENCY": 10, # print status once every so many steps
            "N_STEPS": 100,         # number of training steps
            "WARMUP_STEPS": 20,     # number of warmup steps where learning rate is increased linearly; twice the value in the paper, as in the official transformer repo.
            "STEP": 1,              # the step number, start from 1 to prevent math error in the 'LR' line
            "LR_SCHEDULE": "vaswani",  # the learning rate schedule; see utils.py for learning rate schedule
            "LR_DECAY": None,       # the decay rate for 'exp_decay' schedule
            "START_EPOCH": 0,       # start at this epoch
            ...
            #"CRITERION" = LabelSmoothedCE  # training criterion (loss)
            #"OPTIMIZER": "", #torch.optim.Adam  # optimizer
            "BATCHES_PER_STEP": (8),# perform a training step, i.e. update parameters, once every so many batches
            "TRAINING_CHECKPOINT": None, #"training_runs/" + model_config_name + "/model_train_checkpoints/"  + model_config_name + "_checkpoint.pt", # path to model checkpoint (NAME + ".pt") to resume training, None if none
            "CHECKPOINT_AVG_PREFIX": "step",
            "CHECKPOINT_AVG_SUFFIX": ".pt",  # checkpoint end string to match checkpoints saved for averaging
            "EVAL_GAMES_FOLDER": "training_runs/evaluate_games/" + model_config_name,  # folder where evaluation games are saved in PGN files
            "FINAL_CHECKPOINT": "averaged_" + model_config_name + ".pt", # final checkpoint to be used for eval/inference
            "AVERAGE_STEPS": {491000, 492500, 494000, 495500, 497000, 498500, 500000}
            }
    elif model_config_name=="CT-E-20_run_9":
        run_number = 9
        return {
            "NAME": model_config_name,
            "VOCAB_SIZES": get_vocab_sizes(),
            ...
            #"OPTIMIZER": "", #torch.optim.Adam  # optimizer
            "BATCHES_PER_STEP": (16),# perform a training step, i.e. update parameters, once every so many batches
            "TRAINING_CHECKPOINT": "checkpoint_epoch_300_CT-E-20.pt", # path to model checkpoint (NAME + ".pt") to resume training, None if none
            "CHECKPOINT_AVG_PREFIX": "step",
            "CHECKPOINT_AVG_SUFFIX": ".pt",  # checkpoint end string to match checkpoints saved for averaging
            "EVAL_GAMES_FOLDER": "training_runs/CT-E-20/evaluate_games/run_" + str(run_number),  # folder where evaluation games are saved in PGN files
            "FINAL_CHECKPOINT": "averaged_CT-E-20_run_" + str(run_number) + ".pt", # final checkpoint to be used for eval/inference
            "AVERAGE_STEPS": {9000, 9250, 9550, 9700, 9850, 10000}, #{491000, 492500, 494000, 495500, 497000, 498500, 500000}
            "SAVE_CHECKPOINT_EPOCH_FEQUENCY": 1
            # "MODEL_CHECKPOINT_FOLDER": "training_runs/" + model_config_name + "/model_epoch_checkpoints/"
            }
