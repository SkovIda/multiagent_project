...
if __name__ == '__main__':
    ...
    result_data_filename_postfix = 'epoch_150'
    result_save_filename_prefix = 'IL_model'

    MATChessFormer_Homogeneous_training_loss_csv_filepath = "./results/run_1/tensorboard_training_data" + "_" + result_data_filename_postfix
    ...
    plt.title('Mean Label-smoothed Cross-Entropy Loss\nof the Standard Imitation Learning Model',fontsize="x-large", fontweight='bold')
    ...
    #     plt.plot(epochs[:n_epochs], avg_train_acc_top_k[:n_epochs], color=colors[color_itr], linewidth=2)
    #     color_itr += 1
    ...
    #     epochs, avg_train_acc_top_k = read_csv_training_data(log_dir=csv_logs_dir, filename=filename_train)
    #     plt.plot(epochs[:n_epochs], avg_train_acc_top_k[:n_epochs], color=colors[color_itr], alpha=train_color_alpha, linewidth=2)
    #     color_itr += 1
    for train_acc_tag, valid_acc_tag in zip(train_acc_tags_encoder, valid_acc_tags_encoder):
        filename_val = csv_filename_prefix + valid_acc_tag + filename_postfix
        epochs, avg_val_acc_top_k = read_csv_training_data(log_dir=csv_logs_dir, filename=filename_val)
        plt.plot(epochs[:n_epochs], avg_val_acc_top_k[:n_epochs], color=colors[color_itr], linewidth=2)
    ...
    # plt.legend(['Train top-1', 'Train top-3', 'Train top-5', 'Valid top-1', 'Valid top-3', 'Valid top-5'])#,fontsize="large")
    plt.legend(['Valid top-1', 'Train top-1', 'Valid top-3', 'Train top-3', 'Valid top-5', 'Train top-5'])#,fontsize="large")
    plt.title('Average Top-k Accuracy\nof the Standard Imitation Learning Model',fontsize="x-large", fontweight='bold')
    ...
    result_data_filename_postfix = 'epoch_99'

    result_save_filename_prefix = 'IRL_model'

    MATChessFormer_Heterogeneous_training_loss_csv_filepath = "./results/run_5/tensorboard_training_data" + "_" + result_data_filename_postfix

    csv_filename_prefix = "run-run_5-tag-"
    filename_postfix = ".csv"
    result_save_dir = "./results/run_5/figs"
    ...
    plt.legend(['Train Loss', 'Valid Loss'])
    plt.title('Mean Label-smoothed Cross-Entropy Loss (Policy)\nof the Imitative Reinforcement Learning Model',fontsize="x-large", fontweight='bold')
